...
import { format, subDays, startOfWeek, startOfMonth, isAfter, isSameMonth, parseISO } from 'date-fns';
...
export const StatistikPanel: React.FC<Props> = ({ bpLogs, weightLogs }) => {
  ...
  const weeklyData = useMemo(() => {
    ...
    return last7Days.map(dateStr => {
      const logsForDay = bpLogs.filter(log => log.logged_at.startsWith(dateStr));
  ...
  const monthlyData = useMemo(() => {
    ...
    const today = new Date();
    const currentMonthLogs = bpLogs.filter(log => isSameMonth(new Date(log.logged_at), today));
      ...
      <div className="lg:col-span-8 space-y-6">
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h3 className="text-base font-extrabold text-slate-800 dark:text-slate-100 tracking-tight">Tren 7 Hari Terakhir</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">Rata-rata tensi harian</p>
            </div>
          </div>
      ...
      <div className="lg:col-span-4 space-y-6">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm">
          ...
          <div className="flex items-baseline gap-2 mb-1">
            <span className="text-3xl font-extrabold text-slate-800 dark:text-slate-100 font-mono tracking-tight">
              {monthlyData.count > 0 ? `${monthlyData.sysAvg} / ${monthlyData.diaAvg}` : '- / -'}
            </span>
            <span className="text-xs font-semibold text-slate-400">mmHg</span>
          </div>
          <p className="text-[10px] text-slate-500 font-medium">Berdasarkan {monthlyData.count} pencatatan bulan ini</p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm">
          ...
          {extremes ? (
            <div className="space-y-4">
              <div className="p-3 bg-rose-50 dark:bg-rose-950/20 rounded-2xl border border-rose-100 dark:border-rose-900/30">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[10px] font-bold text-rose-700 dark:text-rose-400 uppercase tracking-wider">Tertinggi</span>
                </div>
              ...
              <div className="p-3 bg-emerald-50 dark:bg-emerald-950/20 rounded-2xl border border-emerald-100 dark:border-emerald-900/30">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[10px] font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-wider">Terendah</span>
                </div>
                <div className="flex items-baseline justify-between">
                  <span className="text-xl font-extrabold text-slate-800 dark:text-slate-100 font-mono">
            ...
            <p className="text-xs text-slate-400 italic">Belum ada data cukup</p>
          )}
        </div>
